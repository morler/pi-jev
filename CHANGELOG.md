...
- An inverted threshold pair (`JEV_COMPACT_DROP` above `JEV_COMPACT_KEEP`) and a request budget at or below the state budget are clamped instead of silently emptying the truncate band or fanning out one request per message. `/jev compact status` prints the effective bands.
- A saved switch that is not a boolean (`"compact": "off"`) is coerced the way the env path coerces it, instead of a truthy string enabling the switch against the file's intent.
