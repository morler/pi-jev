...
### Added
- Skill catalog stripping (`/jev skill-strip [on|off]`, `--jev-skill-strip`, `PI_JEV_SKILL_STRIP`, default off): before each agent turn the generated `<skills>` / `<available_skills>` catalog is removed from the effective system prompt and replaced by one line naming the count and the discovery path, so skills stay discoverable through `jev_find_skill` while the prompt no longer resends the full catalog on every request.
- Search gate: `jev_search_gate` tool (`--jev-search-gate`, `PI_JEV_SEARCH_GATE`, `/jev search-gate [on|off]`) ports the hermes-jev-skills search loop. One call ranks web results with Jev (relevance + injection `noul` per result), screens every result locally for instruction injection, command smuggling, and URL exfiltration before anything leaves the machine, redacts secrets and contact details, judges sufficiency against the question, and picks the next query from the caller's own candidates (`answer` / `search_more` / `propose_queries` / `answer_from_what_we_have` / `unknown`). Fails closed: an outage shortlists nothing (`selected_ids` empty), reports only what the local screen caught, and keeps `decision: "unknown"` — never a claim that the evidence is enough, never a ranking Jev did not produce.
...
- Platform tests no longer read the machine's real persisted platform selection: the `resolvePlatform` test points `JEV_PLATFORM_FILE` at a temp path, so it passes on hosts with any `~/.pi/agent/jev-platform` value.
