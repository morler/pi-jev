...
- **Skill Catalog Stripping (opt-in)**: `--jev-skill-strip` / `PI_JEV_SKILL_STRIP=1` / `/jev skill-strip on` removes the generated `<skills>` / `<available_skills>` catalog from the system prompt before each agent turn and replaces it with one line naming the skill count and the discovery path, so the prompt stops resending the full catalog on every request while skills stay discoverable via `jev_find_skill` and `jev_load_skill`. Off by default.
...
- `/jev skill-strip [on|off]` — Turns Agent Skills catalog stripping from the system prompt on or off.
