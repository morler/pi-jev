...
export async function callJev(
  ...
  const doFetch = call.fetch ?? ((input: any, init?: any) => globalThis.fetch(input, init));

  if (platform === "typesafe") {
    const client = new TypeSafeClient({ apiKey, fetch: doFetch as any });
    const res: any = await client.systemOne(
      { state: call.state as any, questions: call.questions as any, model: call.model },
      { signal: call.signal }
    );
